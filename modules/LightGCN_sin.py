...
class LightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        self.loss = args_config.loss
        self.nesting_list = args_config.nesting_list
        ...
        self.uib_linear = nn.Linear(self.emb_size, 1).to(self.device)
    ...
    def forward(self, cur_epoch, batch=None):
        ...
        if self.loss == 'bpr':
            ...
            return self.create_bce_loss(user, user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs)
        elif self.loss == 'cml':
            return self.create_cml_loss(user, user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs)
        elif self.loss == 'uib':
            return self.create_uib_loss(user, user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs)
        elif self.loss == 'dau':
            return self.create_dau_loss(user, user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs)
        else:
            return self.create_mrl_loss(user, user_gcn_emb[user], item_gcn_emb[pos_item], neg_gcn_embs)
    ...
    def create_mrl_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        # user_gcn_emb: [batch_size, n_hops+1, channel]
        # pos_gcn_embs: [batch_size, n_hops+1, channel]
        # neg_gcn_embs: [batch_size, K, n_hops+1, channel]
        ...
        w_d = 1/len(self.nesting_list)

        for k in range(len(self.nesting_list)):
            d = self.nesting_list[k]

            u_e_ = u_e[:, :d]
            pos_e_ = pos_e[:, :d]
            neg_e_ = neg_e[:, :, :d]

            pos_scores = (u_e_ * pos_e_).sum(dim=-1)
            neg_scores = (u_e_.unsqueeze(dim=1) * neg_e_).sum(dim=-1)
        
            mf_loss += w_d * torch.mean(torch.log(1 + torch.exp(neg_scores - pos_scores.unsqueeze(dim=1)).sum(dim=1)))
    ...
    def create_dau_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        # user_gcn_emb: [batch_size, n_hops+1, channel]
        # pos_gcn_embs: [batch_size, n_hops+1, channel]
        # neg_gcn_embs: [batch_size, K, n_hops+1, channel]
        ...
        align = self.alignment(u_e, pos_e)
        uniform = (self.uniformity(u_e) + self.uniformity(pos_e)) / 2

        mf_loss = align + self.gamma * uniform

        # cul regularizer
        regularize = (torch.norm(user_gcn_emb[:, 0, :]) ** 2
                      + torch.norm(pos_gcn_embs[:, 0, :]) ** 2) / 2  # take hop=0
        emb_loss = self.decay * regularize / batch_size

        return mf_loss + emb_loss, mf_loss, emb_loss
    
    def create_cml_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        # user_gcn_emb: [batch_size, n_hops+1, channel]
        # pos_gcn_embs: [batch_size, n_hops+1, channel]
        # neg_gcn_embs: [batch_size, K, n_hops+1, channel]
        ...
        pos_scores = ((u_e - pos_e) ** 2).sum(dim=-1)
        neg_scores = ((u_e.unsqueeze(dim=1) - neg_e) ** 2).sum(dim=-1)
        
        mf_loss = torch.mean(F.relu(pos_scores.unsqueeze(dim=1) - neg_scores + self.gamma).sum(dim=1))
    ...
    def create_bce_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        # user_gcn_emb: [batch_size, n_hops+1, channel]
        # pos_gcn_embs: [batch_size, n_hops+1, channel]
        # neg_gcn_embs: [batch_size, K, n_hops+1, channel]
        ...
        mf_loss = torch.mean(- torch.log(F.sigmoid(pos_scores)) - torch.log(1 - F.sigmoid(neg_scores)).sum(dim=1))
    ...
    def create_uib_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        # user_gcn_emb: [batch_size, n_hops+1, channel]
        # pos_gcn_embs: [batch_size, n_hops+1, channel]
        # neg_gcn_embs: [batch_size, K, n_hops+1, channel]
        ...
        pos_scores = (u_e * pos_e).sum(dim=-1)
        bu_scores = self.uib_linear(u_e)
        neg_scores = (u_e.unsqueeze(dim=1) * neg_e).sum(dim=-1)
        
        mf_loss = torch.mean(torch.log(1 + torch.exp(bu_scores - pos_scores))) + self.alpha * torch.mean(torch.log(1 + torch.exp(neg_scores - bu_scores.unsqueeze(dim=1)).sum(dim=1)))
