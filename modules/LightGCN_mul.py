...
class LightGCN(nn.Module):
    ...
    def adaptive_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item, d):
        ...
        s_e = s_e.mean(dim=1)[:, :d]  # [batch_size, d]
        p_e = p_e.mean(dim=1)[:, :d]  # [batch_size, d]
        n_e = n_e.mean(dim=2)[:, :, :d]  # [batch_size, n_negs, d]
    ...
    def dynamic_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item, d):
        s_e, p_e = user_gcn_emb[user], item_gcn_emb[pos_item]  # [batch_size, n_hops+1, channel]
        n_e = item_gcn_emb[neg_candidates]  # [batch_size, n_negs, n_hops+1, channel]
    ...
    def curr_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item, cur_epoch, d):
        s_e, p_e = user_gcn_emb[user], item_gcn_emb[pos_item]  # [batch_size, n_hops+1, channel]
        n_e = item_gcn_emb[neg_candidates]  # [batch_size, n_negs, n_hops+1, channel]
    ...
    def mix_negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item, d):
        batch_size = user.shape[0]
        s_e, p_e = user_gcn_emb[user], item_gcn_emb[pos_item]  # [batch_size, n_hops+1, channel]
        if self.pool != 'concat':
        ...
        """hop mixing"""
        scores = (s_e[:, :d].unsqueeze(dim=1) * n_e_[:, :, :d]).sum(dim=-1) # [batch_size, n_negs, n_hops+1]
    ...
    def create_mrl_loss(self, user, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        ...
        for k in range(len(self.nesting_list)):
            ...
            u_e_ = u_e[:, :d]
            pos_e_ = pos_e[:, :d]
            neg_e_ = neg_e[:, k, :d].squeeze(dim=1)

            pos_scores = (u_e_ * pos_e_).sum(dim=-1)
            neg_scores = (u_e_ * neg_e_).sum(dim=-1)
        
            mf_loss +=  w_d * torch.mean(torch.log(1 + torch.exp(neg_scores - pos_scores)))
